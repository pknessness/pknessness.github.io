//https://web.mae.ufl.edu/designlab/Advanced%20Manufacturing/Speeds%20and%20Feeds/Speeds%20and%20Feeds.htm
surface_speeds_feetpmin = {
    "acetal": 250,
    "aluminum": 250,
    "brass": 250,
    "bronze": 100,
    ...
    "magnesium": 300,
    "monel": 25,
    "high_nickel_steel": 50,
    "mild_steel": 100,
    "medium_carbon_steel": 60,
    "tool_steel": 40,
    "forgings": 40,
    "alloy_steel_300_400_brinell": 30,
    "heat_treated_35_40_hrc": 20,
    "heat_treated_40_45_hrc": 20,
    "heat_treated_45_50_hrc": 15,
    "heat_treated_50_55_hrc": 15,
    "free_machining_stainless": 40,
    "work_hardening_stainless": 20,
    "titanium": 20
}

feed_rates_inchptooth = {
    "acetal": [0.008, 0.030],
    "aluminum": [0.004, 0.012],
    "brass": [0.004, 0.012],
    "bronze": [0.002, 0.006],
    "soft_castiron": [0.002, 0.006],
    "medium_castiron": [0.002, 0.006],
    "hard_castiron": [0.0015, 0.004],
    "hastelloy": [0.0015, 0.004],
    "inconel": [0.0015, 0.003],
    "magnesium": [0.004, 0.012],
    "monel": [0.0015, 0.003],
    "high_nickel_steel": [0.002, 0.004],
    "mild_steel": [0.002, 0.008],
    "medium_carbon_steel": [0.002, 0.006],
    "tool_steel": [0.0015, 0.004],
    "forgings": [0.0015, 0.004],
    "alloy_steel_300_400_brinell": [0.0015, 0.004],
    "heat_treated_35_40_hrc": [0.0015, 0.003],
    "heat_treated_40_45_hrc": [0.0015, 0.003],
    "heat_treated_45_50_hrc": [0.0015, 0.003],
    "heat_treated_50_55_hrc": [0.001, 0.002],
    "free_machining_stainless": [0.002, 0.006],
    "work_hardening_stainless": [0.0015, 0.004],
    "titanium": [0.0015, 0.004]
}
...
//added by pknessness
var titles1 = ['RPM', 'WOC(in)', 'DOC(in)', 'MRR', 'IPM']
var titles2 = [' ','IPM', 'RPM', 'MRR', 'HP']
if(useMetric){
    titles1 = ['RPM', 'WOC(mm)', 'DOC(mm)', 'MRR', 'mmPM']
    titles2 = [' ','mmPM', 'RPM', 'MRR', 'kW']
}

function trim_precision(v)
{
...
function finalize()
{
    ...
    var MAX_TORQUE       = trim_precision((HP.max * OneHP * InchesPerFoot) / (2 * Math.PI * PEAK_TQ_RPM.v));    // Max torque computed from derated HP
    ...
    var I                = trim_precision((PI * Math.pow(DIAMETER.v,4)) / 64);        // Second moment of inertia (area) for solid round beam
    ...
    EffectiveDiameter.v        = trim_precision((!ballnose || DOC.v >= RADIUS) ? DIAMETER.v : 2 * Math.sqrt( Math.pow(RADIUS,2) - Math.pow(RADIUS - DOC.v,2) ));
    // Axial chip thinning: Use DOC & actual radius
    AxialChipThinningFactor.v  = trim_precision((!ballnose || DOC.v >= RADIUS) ? 1          : Math.min( 1, Math.sqrt(1-Math.pow(1-(DOC.v/RADIUS),2))));
    // Radial chip thinning: Use WOC & effective diameter
    RadialChipThinningFactor.v = trim_precision((!adaptive || WOC.v >= EffectiveDiameter.v/2) ? 1 : Math.min( 1, Math.sqrt(1-Math.pow(1-(2*WOC.v/EffectiveDiameter.v),2))));
    FeedCorrectionFactor.v     = trim_precision(AxialChipThinningFactor.v * RadialChipThinningFactor.v);
            
    // Equation from: http://zero-divide.net/?shell_id=151&article_id=4257
    ToolEngagementAngle.v      = toDegrees( Math.acos( 1 - Math.min(WOC.v,DIAMETER.v) / RADIUS ) );
    ...
    AvailableTorque.v = trim_precision(Math.min(1,PEAK_TQ_RPM.v / RPM.v) * MAX_TORQUE);
            
    // TODO: More advanced torque curve: lookup table with linear approximation between entries
    TORQUE.max     = trim_precision(/*RESERVE_TORQUE.v * */ AvailableTorque.v);
    AdjustedIPT.v  = trim_precision(CHIPLOAD.v / FeedCorrectionFactor.v);
    IPM.v          = trim_precision((AdjustedIPT.v * TEETH.v * RPM.v));
            
    // Plunge Calculation: Each revolution of the cutter, we should plunge one chipload in depth.
...
function new_compute()
{
    ...
    if ( results.length > 1 )
    {
        ...
        results = results.splice(0,20);                         // Keep only 20 values.
        ...
        if ( typeof google != "undefined" )
        {
            ...
            if(useMetric){
                drawChart(graph, 'chart_div', docOptions_mm);
            }else{
                drawChart(graph, 'chart_div', docOptions_in);
            }
            ...
            if(useMetric){
                drawChart(graph, 'chart_div2', rpmOptions_mm);
            }else{
                drawChart(graph, 'chart_div2', rpmOptions_in);
            }
        }
        else
        {
            ...
            if(useMetric){
                alt.innerHTML = '<table><tr><td>RPM</td><td>mmPM</td><td>WOC</td><td>DOC</td><td>MRR</td></tr>';
            }
            for ( var i = 0; i < results.length; i++ ){
                alt.innerHTML += '<tr><td>' + imperialToDisplay(results[i].RPM, RPM.metToImp) + '</td><td>' + imperialToDisplay(results[i].IPM, IPM.metToImp) + '</td><td>'+ imperialToDisplay(results[i].WOC, WOC.metToImp) + '</td><td>'+ imperialToDisplay(results[i].DOC, DOC.metToImp) + '</td><td>'+ imperialToDisplay(results[i].MRR, MRR.metToImp) + '</td></tr>';
...
function getValue(name,scale)
{
    ...
    highlight( name, isNumeric(val) ? "" : "yellow");   // Highlight non-number inputs
...
function read_values(vs)
{
    Object.keys(vs).forEach(function(n) {
        ...
        p.min = getValue('min_'+n, p.metToImp);
        p.max = Math.max( getValue('in_'+n, p.metToImp), getValue('max_'+n, p.metToImp));
        if ( p.max == 0 ) p.max = 99999;
        if ( p.min > p.max ) highlightInputs(n,"pink");
...
function convertAll()
{
    ...
    if(useMetric){
        ...
        titles1 = ['RPM', 'WOC(in)', 'DOC(in)', 'MRR', 'IPM']
        titles2 = [' ','IPM', 'RPM', 'MRR', 'HP']
    }
...
function display(vs)
{
    Object.keys(vs).forEach(function(n) {
        ...
        if ( item != null )
        {
            var pct = Number((100*p.v/p.max).toPrecision(2))
            item.innerHTML = imperialToDisplay( p.v, p.metToImp ) + ((pct < 1) ? "" : (" <small>(" + pct + "%)</small>"));
                    
            // Highlight values near their limits..
            var close_to_limit = Math.abs(p.v - p.max) <  (p.max * 0.05);
            var beyond_limit = p.v < p.min ||
            p.v > p.max ||
            !isNumeric(p.v);
...
//     Object.keys(vs).forEach(function(n) {
//         var p = vs[n];
//         var min = document.getElementById('min_'+p.name);
//         var max = document.getElementById('max_'+p.name);
        
//         if ( min != null && p.v < p.min ) (min.style.backgroundColor == "") ? min.style.backgroundColor = "cyan" : null;
//         if ( max != null && p.v > p.max ) (max.style.backgroundColor == "") ? max.style.backgroundColor = "cyan" : null;
//     });
// }
...
function reset_compute()
{
    ...
    results  = [];
    if(useMetric){
        drawChart([titles1,['0',0,0,0,0]], 'chart_div', docOptions_mm);
        drawChart([titles2,['',0,0,0,0]], 'chart_div2', rpmOptions_mm);
    }else{
        drawChart([titles1,['0',0,0,0,0]], 'chart_div', docOptions_in);
        drawChart([titles2,['',0,0,0,0]], 'chart_div2', rpmOptions_in);
    }
...
{
    ballnose   = document.getElementById('in_BALLNOSE').checked;
    adaptive   = document.getElementById('in_ADAPTIVE').checked;
...
function amoeba(dimension,point,func,tolerance)
{
    ...
    while (count > 0)
    {
        ...
        if ( check_tolerance(fx[extremes.hi], fx[extremes.lo], tolerance)) break;
...
function optFunc(dimension,point)
{
    ...
    if (validate(parameters))
    return 0 - parameters.MRR.v; // For valid parameters, minimize -MRR
    else
    return 0; // TODO: non-continuous function result - should return affine gradiant pointing toward valid parameter space
    // Maybe L1 distance from Zero-point?
}
